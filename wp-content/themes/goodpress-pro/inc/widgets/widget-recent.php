 ...
 * @since      1.0.0
 */
class goodpress_Recent_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			'classname'   => 'widget-goodpress-recent widget_posts_thumbnail',
			'description' => __( 'Display recent posts with thumbnails.', 'goodpress-pro' )
		);

		// Create the widget.
		parent::__construct(
			'goodpress-recent',                                   // $this->id_base
			__( '&raquo; Recent Posts', 'goodpress-pro' ), // $this->name
	...
	function widget( $args, $instance ) {
		...
		if ( false === ( $recent = get_transient( 'goodpress_recent_widget_' . $this->id ) ) ) {
			...
			// Store the transient.
			set_transient( 'goodpress_recent_widget_' . $this->id, $recent );
	...
	function flush_widget_transient() {
		delete_transient( 'goodpress_recent_widget_' . $this->id );
	}
	...
	function form( $instance ) {
		...
		// Default value.
		$defaults = array(
			'title'     => esc_html__( 'Recent Posts', 'goodpress-pro' ),
			'limit'     => 5,
			'show_date' => true
		);
