 ...
 * @since      1.0.0
 */
class goodpress_Popular_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			'classname'   => 'widget-goodpress-popular widget_posts_thumbnail',
			'description' => __( 'Display popular posts by comments with thumbnails.', 'goodpress-pro' )
		);

		// Create the widget.
		parent::__construct(
			'goodpress-popular',                                   // $this->id_base
			__( '&raquo; Popular Posts', 'goodpress-pro' ), // $this->name
	...
	function widget( $args, $instance ) {
		...
		if ( false === ( $popular = get_transient( 'goodpress_popular_widget_' . $this->id ) ) ) {
			...
			// Store the transient.
			set_transient( 'goodpress_popular_widget_' . $this->id, $popular );
	...
	function flush_widget_transient() {
		delete_transient( 'goodpress_popular_widget_' . $this->id );
	}
	...
	function form( $instance ) {
		...
		// Default value.
		$defaults = array(
			'title'     => esc_html__( 'Popular Posts', 'goodpress-pro' ),
			'limit'     => 5,
			'show_views' => true
		);
	...
	?>
		...
		<p>
			...
			<label for="<?php echo $this->get_field_id( 'show_views' ); ?>">
				<?php _e( 'Display post date?', 'goodpress-pro' ); ?>
			</label>
		</p>
