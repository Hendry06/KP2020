 ...
 * @since      1.0.0
 */
class goodpress_Block_Three_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			'classname'   => 'widget-goodpress-home-three-columns',
			'description' => __( 'Display three-column post blocks. Only use for the "Homepage Content" widget area.', 'goodpress-pro' )
		);

		// Create the widget.
		parent::__construct(
			'happythemes-home-three-columns',         // $this->id_base
			__( '&raquo; Home Three Columns', 'goodpress-pro' ), // $this->name
	...
	function widget( $args, $instance ) {
		...
		echo $before_widget;
			...
			echo '<div class="content-block content-block-3 clear">';
				...
				goodpress_home_posts_left_col( $args, $instance );

				goodpress_home_posts_middle_col( $args, $instance );

				goodpress_home_posts_right_col( $args, $instance );				
	...
	?>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'cat' ); ?>"><?php _e( 'Choose category for column #1', 'goodpress-pro' ); ?></label>
			<select class="widefat" id="<?php echo $this->get_field_id( 'cat' ); ?>" name="<?php echo $this->get_field_name( 'cat' ); ?>" style="width:100%;">
				<?php $categories = get_terms( 'category' ); ?>
				<option value="0"><?php _e( 'All categories &hellip;', 'goodpress-pro' ); ?></option>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'cat_2' ); ?>"><?php _e( 'Choose category for column #2', 'goodpress-pro' ); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'cat_3' ); ?>"><?php _e( 'Choose category for column #3', 'goodpress-pro' ); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'limit' ); ?>">
				<?php _e( 'Number of posts to show', 'goodpress-pro' ); ?>
			</label>
 ...
 * Column left posts
 */
function goodpress_home_posts_left_col( $args, $instance ) {
	...
	$query = apply_filters( 'goodpress_home_three_columns_1_args', $args );
 ...
 * Column middle posts
 */
function goodpress_home_posts_middle_col( $args, $instance ) {
	...
	// Allow dev to filter the post arguments.
	$query = apply_filters( 'goodpress_home_three_columns_2_args', $args );
 ...
 * Column right posts
 */
function goodpress_home_posts_right_col( $args, $instance ) {
	...
	// Allow dev to filter the post arguments.
	$query = apply_filters( 'goodpress_home_three_columns_3_args', $args );
